...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'Lichess');
}
...
const tools: McpToolExport['tools'] = [
  {
    name: 'user',
    description: 'Fetch a Lichess player\'s public profile by username, including ratings across all variants (bullet, blitz, rapid, classical, chess960, etc.) and account metadata.',
    inputSchema: {
      type: 'object',
  ...
  {
    name: 'leaderboards',
    description: 'Fetch the top-10 rated players across every Lichess variant in a single call; returns one ranked list per variant (bullet, blitz, rapid, classical, and more).',
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        _apiKey: { type: 'string', description: 'Lichess Personal API Token (free, https://lichess.org/account/oauth/token).' },
      },
      required: ['scope', '_apiKey'],
    },
  },
];
...
async function callTool(name: string, args: Record<string, unknown>): Promise<unknown> {
  switch (name) {
    ...
    case 'users': {
      ...
      const res = await pwFetch(`${BASE}/users`, {
        method: 'POST',
    ...
    case 'tablebase': {
      ...
      const res = await pwFetch(`${TABLEBASE}/${variant}?${params}`, {
        headers: { Accept: 'application/json', 'User-Agent': UA },
      });
      if (!res.ok) throw await httpError(res, 'Lichess tablebase');
      return res.json();
    }
    case 'opening_explorer': {
      ...
      // Mid-2026 Lichess began requiring auth on every explorer.lichess.ovh
      // request — historical free-anonymous access is gone. A free Personal
      // API Token works (no scopes required for read-only Explorer use).
      const apiKey = args._apiKey as string | undefined;
      delete args._apiKey;
      if (!apiKey) {
        throw new Error('Lichess Opening Explorer now requires a Personal API Token. Create one (free, no scopes needed) at https://lichess.org/account/oauth/token and pass via _apiKey.');
      }
      ...
      const res = await pwFetch(`${EXPLORER}/${scope === 'lichess' ? 'lichess' : scope}?${params}`, {
        headers: {
      ...
      if (res.status === 401) {
        throw new Error('Lichess explorer: 401 — your Personal API Token was rejected. Verify the token at https://lichess.org/account/oauth/token (no scopes are required, but the token must still be active).');
      }
